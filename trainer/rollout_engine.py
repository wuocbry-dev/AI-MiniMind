"""
# Nếu dùng tăng tốc sglang, cần chạy lệnh sau để khởi động mô hình (định dạng transformers) trước:
...
# ===== Lớp cơ sở trừu tượng cho rollout engine =====
...
# ===== Động cơ suy luận SGLang HTTP API =====
class SGLangRolloutEngine(RolloutEngine):
    ...
    def rollout(self, prompt_ids: Tensor, attention_mask: Tensor, num_generations: int, max_new_tokens: int, temperature: float = 0.8) -> RolloutResult:
        # Bỏ padding bên trái, chỉ giữ token hợp lệ
        input_ids_list = []
    ...
    def update_policy(self, model: torch.nn.Module):
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            try:
                ...
                if resp.status_code != 200: print(f"[SGLANG WARNING] update_weights thất bại: {resp.status_code}, {resp.text}")
                ok = resp.status_code == 200
            except Exception as e:
                print(f"[SGLANG WARNING] update_weights lỗi: {e}"); ok = False
...
def create_rollout_engine(
    ...
    if engine_type == "torch":
        ...
        raise ValueError(f"Không hỗ trợ loại engine: {engine_type}")
