...
# Mô hình Critic tùy chỉnh, kế thừa từ MiniMindLM
class CriticModel(MiniMindForCausalLM):
    def __init__(self, params):
        super().__init__(params)
        # Thay thế lm_head bằng lớp tuyến tính xuất ra một giá trị duy nhất
        self.value_head = nn.Linear(params.hidden_size, 1)

    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        # Sử dụng base model để lấy hidden states
        outputs = self.model(input_ids=input_ids, attention_mask=attention_mask, **kwargs)
        hidden_states = self.model.norm(outputs[0])
        # Sử dụng value_head để lấy ước lượng giá trị
...
def ppo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, actor_scheduler, critic_scheduler, reward_model, start_step=0, wandb=None, use_sglang=False):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        with torch.no_grad():  # Giai đoạn Rollout chỉ cần suy luận lấy old_logp và old_values, cắt gradient để tiết kiệm VRAM
            ...
            token_rewards[torch.arange(B, device=args.device)[valid_resp], last_idx[valid_resp]] += rewards[valid_resp]  # Cộng phần thưởng bên ngoài vào cuối
        ...
        for ppo_epoch in range(args.ppo_update_iters):
            ...
            for i in range(0, B, mb_size):
                ...
                # Đồng bộ approx_kl giữa các card, tránh một card break trong khi các card khác tiếp tục gây deadlock DDP
                ...
                # Khi dừng sớm phải đảm bảo vòng lặp forward-backward khép kín, chỉ cắt loss không gián đoạn DDP
                if stop_ppo:
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            # Dùng lm_checkpoint để lưu toàn bộ trạng thái (bao gồm cả critic)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_weight', default='ppo_actor', type=str, help="Tiền tố tên trọng số lưu")
    ...
    parser.add_argument("--learning_rate", type=float, default=3e-7, help="Learning rate Actor")
    parser.add_argument("--critic_learning_rate", type=float, default=5e-7, help="Learning rate Critic")
    ...
    parser.add_argument('--num_hidden_layers', default=8, type=int, help="Số lượng tầng ẩn")
    parser.add_argument('--use_moe', default=0, type=int, choices=[0, 1], help="Có dùng kiến trúc MoE không (0=Không, 1=Có)")
    parser.add_argument('--max_seq_len', default=768, type=int, help="Độ dài tối đa Prompt")
    parser.add_argument("--max_gen_len", type=int, default=1024, help="Độ dài sinh tối đa")
    parser.add_argument("--data_path", type=str, default="../dataset/rlaif.jsonl", help="Đường dẫn dữ liệu RLAIF")
    parser.add_argument("--clip_epsilon", type=float, default=0.2, help="Tham số clip PPO")
    parser.add_argument("--vf_coef", type=float, default=0.5, help="Hệ số value function")
    parser.add_argument("--kl_coef", type=float, default=0.2, help="Hệ số phạt KL")
    parser.add_argument("--gamma", type=float, default=1.0, help="Hệ số discount GAE")
    parser.add_argument("--lam", type=float, default=0.95, help="Tham số lambda GAE")
    parser.add_argument("--cliprange_value", type=float, default=0.2, help="Phạm vi clip value function")
    parser.add_argument("--ppo_update_iters", type=int, default=2, help="Số lần cập nhật lặp lại cho cùng một batch rollout")
    parser.add_argument("--early_stop_kl", type=float, default=0.25, help="Ngưỡng KL cho early stop PPO")
    parser.add_argument("--mini_batch_size", type=int, default=2, help="Kích thước minibatch cho mỗi lần cập nhật PPO")
    parser.add_argument('--from_weight', default='full_sft', type=str, help="Huấn luyện dựa trên trọng số nào")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniMind-PPO", help="Tên dự án wandb")
    parser.add_argument("--use_compile", default=0, type=int, choices=[0, 1], help="Có dùng torch.compile để tăng tốc không (0=Không, 1=Có)")
    parser.add_argument("--debug_mode", action="store_true", help="Có in debug sample huấn luyện không")
    parser.add_argument("--debug_interval", type=int, default=20, help="Trong chế độ debug, cách bao nhiêu step thì in một lần sample")
    parser.add_argument("--thinking_ratio", type=float, default=0.9, help="Bật thinking theo xác suất (0.0~1.0)")
    ...
    parser.add_argument("--sglang_shared_path", type=str, default="./sglang_ckpt_ppo", help="Đường dẫn lưu trữ dùng chung SGLang")
    ...
    # ========== 2. Cấu hình thư mục, tham số model, kiểm tra ckp ==========
