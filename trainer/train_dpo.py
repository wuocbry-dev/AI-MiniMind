...
def dpo_loss(ref_log_probs, policy_log_probs, mask, beta):
    # ref_log_probs và policy_log_probs đều có shape: (batch_size, seq_len)
    ref_log_probs = (ref_log_probs * mask).sum(dim=1)
    policy_log_probs = (policy_log_probs * mask).sum(dim=1)

    # Tách dữ liệu chosen và rejected
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_weight', default='dpo', type=str, help="Tiền tố tên trọng số lưu")
    parser.add_argument("--epochs", type=int, default=1, help="Số epoch huấn luyện")
    parser.add_argument("--batch_size", type=int, default=4, help="batch size")
    parser.add_argument("--learning_rate", type=float, default=4e-8, help="Learning rate ban đầu (khuyến nghị <=5e-8 để tránh quên)")
    ...
    parser.add_argument('--max_seq_len', default=1024, type=int, help="Độ dài cắt tối đa khi huấn luyện (tiếng Trung 1 token ≈ 1.5~1.7 ký tự)")
    parser.add_argument('--use_moe', default=0, type=int, choices=[0, 1], help="Có dùng kiến trúc MoE không (0=không, 1=có)")
    parser.add_argument("--data_path", type=str, default="../dataset/dpo.jsonl", help="Đường dẫn dữ liệu huấn luyện DPO")
    ...
    parser.add_argument('--beta', default=0.15, type=float, help="Tham số beta trong DPO")
    parser.add_argument("--use_wandb", action="store_true", help="Có dùng wandb không")
    parser.add_argument("--wandb_project", type=str, default="MiniMind-DPO", help="Tên dự án wandb")
    ...
    # ========== 5. Định nghĩa mô hình và mô hình tham chiếu ==========
    model, tokenizer = init_model(lm_config, args.from_weight, device=args.device)
    Logger(f'Tổng số tham số mô hình chính sách: {sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    # Khởi tạo mô hình tham chiếu (ref_model được đóng băng)
    ref_model, _ = init_model(lm_config, args.from_weight, device=args.device)
    ref_model.eval()
    ref_model.requires_grad_(False)
    Logger(f'Tổng số tham số mô hình tham chiếu: {sum(p.numel() for p in ref_model.parameters()) / 1e6:.3f} M')
