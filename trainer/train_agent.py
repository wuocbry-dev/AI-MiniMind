...
# ======== Định nghĩa công cụ ========
TOOLS = [
    {"type": "function", "function": {"name": "calculate_math", "description": "Tính biểu thức toán học", "parameters": {"type": "object", "properties": {"expression": {"type": "string"}}, "required": ["expression"]}}},
    ...
    {"type": "function", "function": {"name": "get_current_weather", "description": "Lấy thời tiết", "parameters": {"type": "object", "properties": {"location": {"type": "string"}}, "required": ["location"]}}},
    {"type": "function", "function": {"name": "get_current_time", "description": "Lấy thời gian hiện tại", "parameters": {"type": "object", "properties": {"timezone": {"type": "string", "default": "Asia/Ho_Chi_Minh"}}, "required": []}}},
    {"type": "function", "function": {"name": "get_exchange_rate", "description": "Tra cứu tỷ giá", "parameters": {"type": "object", "properties": {"from_currency": {"type": "string"}, "to_currency": {"type": "string"}}, "required": ["from_currency", "to_currency"]}}},
...
# ======== Dữ liệu giả lập ========
WEATHER_DATA = {"Ha Noi": ("28°C", "Nắng"), "TP.HCM": ("35°C", "Nóng bức"), "Da Nang": ("32°C", "Nắng"), "Hai Phong": ("30°C", "Nắng"), "Can Tho": ("33°C", "Nhiều mây"), "Hue": ("27°C", "Mưa nhỏ"), "Bien Hoa": ("34°C", "Nhiều mây"), "Nha Trang": ("30°C", "Nắng"), "Quy Nhon": ("29°C", "Gió lớn"), "Vung Tau": ("31°C", "Ẩm"), "Tokyo": ("12°C", "Nắng"), "New York": ("8°C", "Nhiều mây"), "London": ("5°C", "Mưa nhỏ"), "Paris": ("10°C", "Ẩm"), "Sydney": ("25°C", "Quang đãng")}
...
TRANSLATE_DATA = {("Xin chao the gioi", "english"): "Hello World", ("Good morning", "vietnamese"): "Chao buoi sang", ("Hom nay thoi tiet that dep", "english"): "The weather is nice today", ("I love programming", "vietnamese"): "Toi thich lap trinh", ("May hoc rat thu vi", "english"): "Machine learning is interesting", ("Happy birthday", "vietnamese"): "Chuc mung sinh nhat"}
...
MOCK_RESULTS = {
    ...
    "get_current_weather": lambda args: (lambda w: {"city": args.get("location"), "temperature": w[0], "humidity": "65%", "condition": w[1]})(WEATHER_DATA.get(args.get("location"), ("22°C", "Nắng"))),
...
# ======== Phân tích và thực thi gọi công cụ ========
...
def rollout_single(rollout_engine, tokenizer, messages, tools, max_turns=3, max_new_tokens=256, thinking_ratio=0.5, device="cuda"):
    ...
    for turn in range(max_turns):
        ...
        for call in calls:
            ...
            result_str = (json.dumps(result, ensure_ascii=False) if result else '{"error": "công cụ không tồn tại"}')[:2048]  # Đề phòng kết quả quá lớn làm tràn tokenizer
...
def calculate_rewards(prompts, completions, gt_batch, tools_batch, num_gen, reward_model=None, device="cuda", turn_outputs_batch=None, unfinished_batch=None):
    ...
    for idx, response in enumerate(completions):
        ...
        tool_calls = []
        for turn_answer in turn_answers: tool_calls.extend(parse_tool_calls(turn_answer))  # Phân tích gọi tool
        reward -= 0.5 * sum(abs(turn.count('<tool_call>') - turn.count('</tool_call>')) for turn in turn_answers)  # Trừ điểm thẻ không khớp
        # -------- Không có gọi công cụ: thưởng định dạng + reward --------
        if not tool_calls:
            reward += 0.5 if 5 <= len(response.strip()) <= 800 else -0.5  # Điểm độ dài
            if '</think>' in response:
                think, answer = response.split('</think>', 1)
                reward += 1.0 if 20 <= len(think.strip()) <= 300 else -0.5  # Điểm độ dài suy nghĩ
                reward += 0.25 if response.count('</think>') == 1 else -0.25  # Điểm đóng thẻ suy nghĩ
                answer = answer.strip()
            if reward_model is not None:
                ...
                reward += score  # Điểm RM
            reward -= rep_penalty(answer)
            rewards[idx] = max(min(reward, 3.0), -3.0)  # Clip tổng điểm
        # -------- Có gọi công cụ: thưởng kết quả thực thi --------
        else:
            ...
            tool_gap = abs(valid_call_count - len(gt)) + max(0, len(tool_calls) - valid_call_count)  # Chênh lệch số lượng tool
            reward += 0.5 if tool_gap == 0 else -0.5 * tool_gap  # Điểm cân bằng tool
            ...
            if gt: reward += 2.5 * len(verified) / len(gt)  # Điểm GT
            if unfinished: reward -= 0.5  # Trừ điểm chưa hoàn thành
            reward -= rep_penalty(final_text if final_text else answer)
            rewards[idx] = max(min(reward, 3.0), -3.0)  # Clip tổng điểm
    return rewards

# ================================ Công cụ và Reward = Kết thúc ================================
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_weight', default='agent', type=str, help="Tên trọng số lưu")
    ...
    parser.add_argument("--dtype", type=str, default="bfloat16", help="Kiểu dữ liệu bfloat16/float16")
    ...
    parser.add_argument("--log_interval", type=int, default=1, help="Chu kỳ in log")
    parser.add_argument("--save_interval", type=int, default=10, help="Chu kỳ lưu mô hình")
    parser.add_argument('--hidden_size', default=768, type=int, help="Kích thước tầng ẩn của mô hình")
    parser.add_argument('--num_hidden_layers', default=8, type=int, help="Số tầng của mô hình")
    parser.add_argument('--use_moe', default=0, type=int, choices=[0, 1], help="Có dùng MoE không")
    parser.add_argument('--max_seq_len', default=1024, type=int, help="Độ dài chuỗi tối đa")
    parser.add_argument("--max_gen_len", type=int, default=768, help="Độ dài sinh tối đa cho mỗi lần")
    parser.add_argument("--max_total_len", type=int, default=2500, help="Giới hạn tổng độ dài phía huấn luyện")
    parser.add_argument("--data_path", type=str, default="../dataset/agent_rl.jsonl", help="Đường dẫn dữ liệu huấn luyện")
    parser.add_argument("--num_generations", type=int, default=4, help="Số mẫu sinh cho mỗi prompt")
    parser.add_argument("--beta", type=float, default=0.1, help="Hệ số phạt KL")
    parser.add_argument("--loss_type", type=str, default="cispo", choices=["grpo", "cispo"], help="Loại loss")
    parser.add_argument("--epsilon", type=float, default=0.2, help="Epsilon clip PPO của GRPO")
    parser.add_argument("--epsilon_high", type=float, default=5.0, help="Giới hạn trên epsilon")
    parser.add_argument('--from_weight', default='full_sft', type=str, help="Tên trọng số pretrain cần tải")
    parser.add_argument('--from_resume', default=0, type=int, choices=[0, 1], help="Có khôi phục từ checkpoint không")
    parser.add_argument("--use_wandb", action="store_true", help="Có ghi log với wandb không")
    parser.add_argument("--wandb_project", type=str, default="MiniMind-Agent-RL", help="Tên dự án wandb")
    parser.add_argument("--use_compile", default=0, type=int, choices=[0, 1], help="Có dùng torch.compile không")
    parser.add_argument("--debug_mode", action="store_true", help="Chế độ debug")
    parser.add_argument("--debug_interval", type=int, default=20, help="Chu kỳ log debug")
    parser.add_argument("--thinking_ratio", type=float, default=0.1, help="Bật thinking theo xác suất (0.0~1.0)")
    parser.add_argument("--reward_model_path", type=str, default="../../internlm2-1_8b-reward", help="Đường dẫn mô hình Reward")
    parser.add_argument("--rollout_engine", type=str, default="torch", choices=["torch", "sglang"], help="Loại rollout engine")
    parser.add_argument("--sglang_base_url", type=str, default="http://localhost:8998", help="URL máy chủ SGLang")
    parser.add_argument("--sglang_model_path", type=str, default="../model", help="Đường dẫn tokenizer SGLang")
    parser.add_argument("--sglang_shared_path", type=str, default="./sglang_ckpt_agent", help="Đường dẫn lưu trữ dùng chung SGLang")
    ...
    # Rollout engine (có thể thay thế, chỉ phụ trách suy luận policy)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if skip > 0:
            Logger(f'Epoch [{epoch+1}/{args.epochs}]: Bỏ qua {start_step} bước')
