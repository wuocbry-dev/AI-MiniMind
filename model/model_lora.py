...
# Định nghĩa cấu trúc mạng LoRA
class LoRA(nn.Module):
    def __init__(self, in_features, out_features, rank):
        super().__init__()
        self.rank = rank  # Rank của LoRA, điều khiển kích thước ma trận hạng thấp
        self.A = nn.Linear(in_features, rank, bias=False)  # Ma trận hạng thấp A
        self.B = nn.Linear(rank, out_features, bias=False)  # Ma trận hạng thấp B
        # Khởi tạo Gaussian cho ma trận A
        self.A.weight.data.normal_(mean=0.0, std=0.02)
        # Khởi tạo toàn 0 cho ma trận B
