...
history_messages_num = 0  # Bắt buộc là số chẵn (Q+A); 0 thì không kèm hội thoại trước
while True:
    query = input('[Q]: ')
    ...
    response = client.chat.completions.create(
        ...
        max_tokens=2048,
        top_p=0.8,
        extra_body={"chat_template_kwargs": {"open_thinking": True}, "reasoning_effort": "medium"} # Bật/tắt suy nghĩ
    )
    if not stream:
