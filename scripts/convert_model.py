...
def convert_torch2transformers_minimind(torch_path, transformers_path, dtype=torch.float16):
    ...
    lm_model = lm_model.to(dtype)  # Chuyển đổi độ chính xác trọng số mô hình
    model_params = sum(p.numel() for p in lm_model.parameters() if p.requires_grad)
    print(f'Số tham số mô hình: {model_params / 1e6} triệu = {model_params / 1e9} B (Billion)')
    lm_model.save_pretrained(transformers_path, safe_serialization=False)
    tokenizer = AutoTokenizer.from_pretrained('../model/')
    tokenizer.save_pretrained(transformers_path)
    # ======= Cách tương thích transformers-5.0 với bản cũ =======
    ...
    print(f"Mô hình đã được lưu ở định dạng Transformers-MiniMind: {transformers_path}")


# Tương thích hệ sinh thái cấu trúc QwenForCausalLM/LlamaForCausalLM
def convert_torch2transformers(torch_path, transformers_path, dtype=torch.float16):
    ...
    qwen_model = qwen_model.to(dtype)  # Chuyển đổi độ chính xác trọng số mô hình
    ...
    if int(transformers.__version__.split('.')[0]) >= 5:
        ...
        config['rope_theta'] = lm_config.rope_theta; config['rope_scaling'] = None; del config['rope_parameters']
        json.dump(config, open(config_path, 'w', encoding='utf-8'), indent=2, ensure_ascii=False)
    print(f"Mô hình đã được lưu ở định dạng Transformers: {transformers_path}")


def convert_transformers2torch(transformers_path, torch_path):
    model = AutoModelForCausalLM.from_pretrained(transformers_path, trust_remote_code=True)
    torch.save({k: v.cpu().half() for k, v in model.state_dict().items()}, torch_path)
    print(f"Mô hình đã được lưu ở định dạng PyTorch: {torch_path}")
...
def convert_merge_base_lora(base_torch_path, lora_path, merged_torch_path):
    ...
    print(f"Đã hợp nhất LoRA và lưu ở định dạng PyTorch cho mô hình gốc: {merged_torch_path}")
...
def convert_json_to_jinja(json_file_path, output_path):
    ...
    print(f"Mẫu đã được lưu thành tệp jinja: {output_path}")
